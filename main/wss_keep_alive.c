...
static uint64_t _tick_get_ms(void) { return esp_timer_get_time() / 1000; }
...
static uint64_t get_max_delay(wss_keep_alive_t h) {
  ...
  for (int i = 0; i < h->max_clients; ++i) {
    if (h->clients[i].type == CLIENT_ACTIVE) {
      uint64_t check_this_client_at =
          h->clients[i].last_seen + h->keep_alive_period_ms;
...
static void keep_alive_task(void *arg) {
  ...
  while (run_task) {
    if (xQueueReceive(keep_alive_storage->q, (void *)&client_action,
                      get_max_delay(keep_alive_storage) / portTICK_PERIOD_MS) ==
        pdTRUE) {
      ...
      case CLIENT_FD_REMOVE:
        ...
        break;
      case CLIENT_UPDATE:
        if (!update_client(keep_alive_storage, client_action.fd,
...
esp_err_t wss_keep_alive_client_is_active(wss_keep_alive_t h, int fd) {
  client_fd_action_t client_fd_action = {
      .fd = fd, .type = CLIENT_UPDATE, .last_seen = _tick_get_ms()};
  if (xQueueSendToBack(h->q, &client_fd_action, 0) == pdTRUE) {
    return ESP_OK;
  }
  return ESP_FAIL;
...
void *wss_keep_alive_get_user_ctx(wss_keep_alive_t h) { return h->user_ctx; }
